...
List the ids of records which must never be deleted, for example a default language or the main administrator account. Every other record in the section can still be deleted.
...
```php app/Models/Package/Package.php
protected $settings = [
...
| `setAdminRowResponse` | a single record, when it is opened in the form |
| `setAdminRowsResponse` | the rows of the table |
