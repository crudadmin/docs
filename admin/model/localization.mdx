...
With mirrored content the record exists once in the database, but each language version holds a different value in the column.

Enable it with the `locale` parameter on the field you want to translate.

```php
protected $fields = [
    'name' => 'name:Project title|type:string|locale',
];
```
